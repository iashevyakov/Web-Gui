...
public class DetailServiceImpl implements DetailService {
    ...
    public void addDetail(String detail, String firm, String node) {
        ...
        if (checkResult) {
            if (FirmVerify.checkFirm(firm) != null) {//если существует такая фирма
                if (NodeVerify.checkNode(node) != null) { //если существует такой узел
                    if (DetailVerify.checkDetail(detail) == null) {
                        //если еще не существует такой детали, добавляем
                        detailDao.addDetail(detail, firm, node);
                    } else {
                        Err.message = "THE DETAIL IS ALREADY IN OUR CATALOG!";
                    }
                } else {
                    Err.message = "WE DON'T BUY DETAILS OF THIS NODE";
                }
            } else {
    ...
    public void deleteDetail(String detail) {
        ...
        if (checkResult) {//проверка на правильность введения данных.
            if (DetailVerify.checkDetail(detail) != null) {
                detailDao.deleteDetail(detail);
                //если такая деталь есть, удаляем её
            } else {//если нет, говорим об этом.
                Err.message = "THERE ISN'T SUCH DETAIL IN OUR WAREHOUSE!";
            }
        } else {//данные не прошли проверку, говорим об этом.
            Err.message = "CHECK YOUR ENTERED DATA!";
        }
    }
